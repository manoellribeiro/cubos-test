...
import '../../../../../core/functions/getGenreNameById.dart';
...
class ListMoviesController = _ListMoviesControllerBase
    with _$ListMoviesController;
...
abstract class _ListMoviesControllerBase with Store {
  ...
  bool thereAreMoreMovies() =>
      lastDiscoverMoviesApiResponse.page <
      lastDiscoverMoviesApiResponse.totalPages;

  @action
  Future fetchMoreMovies() async {
    if (thereAreMoreMovies()) {
      final moviesResult = await getMoviesResults.call(
          chooseGenreIdByTabBarIndex(selectedTabBarIndex),
          lastDiscoverMoviesApiResponse.page + 1);
      moviesResult.fold((errorResult) {
        atualState = ListPageStates.failure;
        failure = errorResult;
      }, (successResult) {
  ...
  List<MovieResults> get moviesResultListFiltered {
    if (movieTitleFilter.isEmpty) {
      ...
      return moviesResultList
          .where((movieResult) => movieResult.title
              .toLowerCase()
              .contains(movieTitleFilter.toLowerCase()))
          .toList();
    }
  }
  ...
  String createGenreString(List<int> genreIds) {
    List<String> genresList =
        genreIds.map((genreId) => getGenreNameById(genreId)).toList();
    String genreString = genresList.join(' - ');
    return genreString;
  }
}

enum ListPageStates { loading, success, failure }
